...
def get_model(model_path=None, num_branches=1):
	...
	model.add(binary_conv(nfilters=96, ch_in=3, k=7, strides=(2,2), padding='valid', input_shape=[107,107,3], name='conv1'))
	# model.add(LRN())
	...
	model.add(binary_conv(nfilters=256, ch_in=96, k=5, strides=(2,2), padding='valid', name='conv2'))
	# model.add(LRN())
	model.add(BatchNormalization(axis=-1, momentum=batch_norm_alpha, epsilon=batch_norm_eps))
	model.add(Residual_sign(levels=resid_levels))
	model.add(MaxPooling2D(pool_size=(3,3), strides=(2,2)))
	...
	model.add(binary_conv(nfilters=512, ch_in=256, k=3, strides=(1,1), padding='valid', name='conv3'))
	model.add(BatchNormalization(axis=-1, momentum=batch_norm_alpha, epsilon=batch_norm_eps))
	model.add(Residual_sign(levels=resid_levels))
	...
	model.add(binary_dense(n_in=512*3*3, n_out=512, name='fc4'))
	model.add(BatchNormalization(axis=-1, momentum=batch_norm_alpha, epsilon=batch_norm_eps))
	model.add(Residual_sign(levels=resid_levels))

	model.add(Dropout(0.5))
	# model.add(Dense(512, activation='relu', name='fc5'))
	model.add(binary_dense(n_in=512, n_out=512, name='fc5'))
	model.add(BatchNormalization(axis=-1, momentum=batch_norm_alpha, epsilon=batch_norm_eps))
	model.add(Residual_sign(levels=resid_levels))
	...
	if model_path is not None:
		...
		elif os.path.splitext(model_path)[1] == '':
			# Load weights into the new model
			model.load_weights(opts['model_path']+'.h5')
		else:
